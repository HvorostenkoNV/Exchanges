...
/** ***********************************************************************************************
 * Application participant ContractingPartiesAddresses1C
 *
 * @package exchange_exchange_participants
 * @author  Hvorostenko
 *************************************************************************************************/
class ContractingPartiesAddresses1C extends AbstractParticipant
{
    private static $addressesTypes =
        [
            'REGISTERED'    => 'Юридический адрес',
            'PRIMARY'       => 'Фактический адрес',
            'HOME'          => 'Адрес регистрации',
    ...
    protected function readProvidedData() : array
    {
        ...
        $logMessagePrefix   = '1C contractingparties addresses provided data reading';
    ...
    {
        $logger             = Logger::getInstance();
        $logMessagePrefix   = '1C contractingparties addresses providing data for delivery';
    ...
    private function getProvidedDataFile() : SplFileInfo
    {
        ...
        $receivedDataFolderParam    = $config->getParam('participants.contractingparties.addresses.1c.receivedDataPath');
    ...
    private function getAnswerDataFile() : SplFileInfo
    {
        $config                     = Config::getInstance();
        $tempFolderParam            = $config->getParam('structure.tempFolder');
        $returnedDataFolderParam    = $config->getParam('participants.contractingparties.addresses.1c.returnedDataPath');
    ...
    {
        $requisiteId    = (string)  ($itemData['ИдРеквизитаКонтрагента1С']  ?? '');
        $typeRaw        = (string)  ($itemData['Тип']                       ?? '');
        $type           = array_search($typeRaw, self::$addressesTypes);
        $type           = $type !== false ? $type : '';
        $addressId      = strlen($requisiteId) > 0 && strlen($type) > 0
            ? "$requisiteId-$type"
            : '';

        $itemData['Ид']                     = $addressId;
        $itemData['Тип']                    = $type;
        $itemData['ИдАдресаКонтрагента1С']  = $addressId;
