...
/** ***********************************************************************************************
 * Application participant ContractingPartiesRequisites1C
 *
 * @package exchange_exchange_participants
 * @author  Hvorostenko
 *************************************************************************************************/
class ContractingPartiesRequisites1C extends AbstractParticipant
{
    ...
    protected function readProvidedData() : array
    {
        ...
        $logMessagePrefix   = '1C contractingparties requisites provided data reading';
    ...
    {
        $logger             = Logger::getInstance();
        $logMessagePrefix   = '1C contractingparties requisites providing data for delivery';
    ...
    private function getProvidedDataFile() : SplFileInfo
    {
        ...
        $receivedDataFolderParam    = $config->getParam('participants.contractingparties.requisites.1c.receivedDataPath');
    ...
    private function getAnswerDataFile() : SplFileInfo
    {
        $config                     = Config::getInstance();
        $tempFolderParam            = $config->getParam('structure.tempFolder');
        $returnedDataFolderParam    = $config->getParam('participants.contractingparties.requisites.1c.returnedDataPath');
    ...
    private function convertProvidedItemData(array $itemData) : array
    {
        ...
        $itemData['Тип']                        = $type;
        $itemData['ИдРеквизитаКонтрагента1С']   = (string) ($itemData['Ид'] ?? '');
