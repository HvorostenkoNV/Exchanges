...
use
    ...
    Main\Helpers\Config;
/** ***********************************************************************************************
 * Application participant ContractingPartiesRequisitesBitrix
 *
 * @package exchange_exchange_participants
 * @author  Hvorostenko
 *************************************************************************************************/
class ContractingPartiesRequisitesBitrix extends AbstractParticipant
{
    private static $requisitesTypes =
        [
            'COMPANY'       => '#CRM_REQUISITE_PRESET_DEF_RU_COMPANY#',
            'INDIVIDUAL'    => '#CRM_REQUISITE_PRESET_DEF_RU_INDIVIDUAL#',
            'PERSON'        => '#CRM_REQUISITE_PRESET_DEF_RU_PERSON#',
            'OTHER'         => '#CRM_REQUISITE_PRESET_DEF_RU_OTHER#'
        ];
    /** **********************************************************************
    ...
    protected function readProvidedData() : array
    {
        ...
        $logMessagePrefix           = 'BITRIX contractingparties requisites provided data reading';
        $exportRequestUrlTemplate   = $config->getParam('participants.contractingparties.requisites.bitrix.exportRequestUrl');
        $userLogin                  = $config->getParam('participants.contractingparties.requisites.bitrix.userLogin');
        $userPassword               = $config->getParam('participants.contractingparties.requisites.bitrix.userPassword');
    ...
    protected function provideDataForDelivery(array $data) : bool
    {
        $logger                     = Logger::getInstance();
        $config                     = Config::getInstance();
        $logMessagePrefix           = 'BITRIX contractingparties requisites providing data for delivery';
        $importRequestUrlTemplate   = $config->getParam('participants.contractingparties.requisites.bitrix.importRequestUrl');
        $userLogin                  = $config->getParam('participants.contractingparties.requisites.bitrix.userLogin');
        $userPassword               = $config->getParam('participants.contractingparties.requisites.bitrix.userPassword');
    ...
    private function convertProvidedItemData(array $itemData) : array
    {
        ...
        $type       = array_search($typeRaw, self::$requisitesTypes);
    ...
    {
        $typeRaw    = (string) ($itemData['Тип'] ?? '');
        $type       = self::$requisitesTypes[$typeRaw] ?? '';
